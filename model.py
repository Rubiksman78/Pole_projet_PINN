...
def define_net(num_inputs,num_outputs,ub,lb):
    ...
    for neurons in [500,500,500,500,500,500]: #Ici 4 couches denses mais on peut en mettre moins
...
class PINN(models.Model):
    ...
    **kwargs):
    ...
    def compute_loss(self,X_r,X_data,u_data):
        ...
        for i in range(len(X_data)):
            if i == 1:
                ...
                loss_b2 += tf.reduce_mean(tf.square(u_data[i] - \
                    tf.expand_dims(v_pred,axis=-1)))
    ...
    def train_step(self,X_r,X_data,u_data):
        with tf.GradientTape() as tape:
            Ju = compute_Ju(tf.concat(X_data,axis=0),self.model)
            Jr = compute_Jr(X_r,self.model,self.c,self.dimension)
            Kuu = compute_Kuu(tf.concat(X_data,axis=0),Ju)
            Krr = compute_Krr(X_r,Jr)
            lambda_b = compute_eigen(Kuu,Krr,iskr=False)
            lambda_r = compute_eigen(Kuu,Krr,iskr=True)
            ...
            loss = self.init_weight * loss_i + \
                self.bound_weight * (loss_b1 + loss_b2) + \
        ...
        return loss_i,loss_b1,loss_b2,loss_r,lambda_b,lambda_r