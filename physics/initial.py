...
def set_training_data(tmin,tmax,xmin,xmax,dimension,N_0,N_b,N_r,speed=True):
    ...
    x_0 = tf.random.uniform((N_0,dimension), lb[1], ub[1], dtype=DTYPE) #uniform distribution initial points
    X_0 = t_0
    for i in range(dimension):
        X_0 = tf.concat([X_0, tf.expand_dims(x_0[:,i],axis=-1)], axis=1) #getting X0 = (t0,x01,...,x0k)
    ...
    #Boundary conditions
    t_b = tf.random.uniform((N_b,1), lb[0], ub[0], dtype=DTYPE) #time uniformly sampled
    x_b = lb[1] + (ub[1] - lb[1]) * tf.keras.backend.random_bernoulli((N_b,dimension), 0.5, dtype=DTYPE) #x_b in either lower or upper bound
    X_b = t_b
    for i in range(dimension):
    ...
    t_r = tf.random.uniform((N_r,1), lb[0], ub[0], dtype=DTYPE) #uniform t and x for residual
